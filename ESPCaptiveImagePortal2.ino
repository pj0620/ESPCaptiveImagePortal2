...
void setup()
{
  ...
  // pj added for microsoft verification
  server.on("/generate_204",handleInternetCheck);  //Android captive portal. Maybe not needed. Might be handled by notFound handler.
  server.on("/fwlink", handleInternetCheck);  //Microsoft captive portal. Maybe not needed. Might be handled by notFound handler.
  server.on("/ncsi.txt", handleInternetCheck);
  server.on("/msftncsi", handleInternetCheck);
  server.on("/connecttest.txt", handleInternetCheck);
...
//  Serial.printf("method: %s", request->method().c_str());
  Serial.printf("url: %s\n", request->url().c_str());
  Serial.printf("host: %s\n", request->host().c_str());
  Serial.printf("contentType: %s\n", request->contentType().c_str());
  Serial.printf("contentLength: %z\n", request->contentLength());
//  Serial.printf("multipart: %s", request->multipart().c_str());
}

void handleInternetCheck(AsyncWebServerRequest *request) {
  Serial.println("");
  Serial.println("handleInternetCheck");
...
//  request->send(204, "text/plain", buffer);   
